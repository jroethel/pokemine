const fs = require('fs');
const { getTextProvider, providers } = require('./text-providers');
const { geminiText } = require('./gemini');

// Fallback order for a hung/throttled provider (issue #18): try the next one
// that has an API key, once, instead of erroring straight to the kid.
const FALLBACK_ORDER = ['gemini', 'anthropic', 'openai'];
const isTransient = e => /timeout|timed out|abort|high demand|overloaded|429|503|rate limit/i.test(e.message || e.name || '');
const siblingProvider = name => FALLBACK_ORDER.find(p => p !== name && providers[p]);

const SYSTEM = `You invent silly Pokemon-style creatures for a kid aged 12-14.
...
const ART_STYLE = 'Cel-shaded official Pokemon-style game art in the Ken Sugimori watercolor style: simple readable silhouette, bold slightly-varied outlines, 1-2 midtone main colors plus one accent, soft purple-tinted shadows, basic highlights, full body, single creature, plain white background. Do not write the creature\'s name or any text, letters, numbers, logos, or watermarks anywhere in the image. Do your best to honor specifics and intent of the kid\'s prompt.';

// Per-stage stat bands and the rare special tier for final evolutions.
// Tweak here: bands are [floor, ceiling]; odds are absolute (sum = 35%).
const STAGES = {
  hp:  { 1: [30, 120], 2: [60, 160], 3: [90, 200] },   // [floor, ceiling]
  dmg: { 1: [10, 60],  2: [30, 120], 3: [60, 200] },
  special: {
    odds: { VMAX: 0.15, EX: 0.12, Mega: 0.08 },      // absolute, sums to 35%; VMAX < EX < Mega
    variants: { // floors guarantee a special always out-stats a normal stage 3
      VMAX: { hpMin: 220, hpMax: 260, dmgMin: 220, dmgMax: 260, label: 'VMAX', art: 'crackling electric-blue energy, radiant silver aura, sparks flying, dynamic stadium background with storm-lit clouds and lightning' },
      EX:   { hpMin: 240, hpMax: 300, dmgMin: 240, dmgMax: 300, label: 'EX',   art: 'wreathed in golden fire, glowing prismatic divine radiance, radiant sunburst temple background' },
...
function stageShape(stageNo, variant) {
  ...
  "moves": [{"name": "...", "damage": <${dmgLo}-${dmgMax}, multiple of 10>, "text": "one short Pokemon inspired skill"},
            {"name": "...", "damage": <${dmgLo}-${dmgMax}, multiple of 10>, "text": "one short Pokemon inspired power move"}],
  ...
  "backstory": "3-5 sentence funny origin story; ground it canon like a half-forgotten folk legend when it fits"
}`;
}

async function callJSON(prompt, opts = {}) {
  let providerName = opts.textProvider || process.env.TEXT_PROVIDER || 'gemini';
  let hopped = false;
  let lastErr;
  for (let attempt = 0; attempt < 3; attempt++) {
    let text;
    try {
      text = await getTextProvider(providerName).complete({ system: buildSystem(), user: prompt });
    } catch (e) {
      lastErr = e;
      const fallback = !hopped && isTransient(e) && siblingProvider(providerName);
      if (fallback) {
        opts.onFallback?.(providerName, fallback);
        providerName = fallback; hopped = true; continue;
      }
      throw e;
    }
    try {
...
function extractJSON(apiResponse) {
  return JSON.parse(geminiText(apiResponse));
}
...
// The 30% jackpot: only the evolution INTO stage 3 rolls; result is stored on the stage.
...
function evolvedStage(record, guidance, variant, opts = {}) {
...
${guidance ? `The kid wants the evolution to be: ${guidance}\n` : ''}${variant ? `This is a rare ${variant} mega-style evolution - dramatically more 
powerful and epic.\n Have this picture include an appropriate scenic background.` : ''}HP and move damage must be higher than before (hp was ${prev.hp}).  
Write a NEW backstory that clearly continues the previous stage's.
Reply with JSON: ${stageShape(stageNo, variant)}`, opts);
    validateStage(data);
