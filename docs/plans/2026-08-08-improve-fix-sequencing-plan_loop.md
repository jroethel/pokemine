...
- Deliverables die with a passing worktree (it is deleted on pass), so each `check` exports the worker's uncommitted edits with the patch-export pattern `git add -A; git reset -q -- node_modules 2>/dev/null; git diff --cached > /tmp/improve-fix-sequencing/exports/<key>.patch`, and the orchestrator applies that patch on the integration branch after review.
