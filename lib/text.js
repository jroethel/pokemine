...
const STAGES = {
  ...
  special: {
    odds: { EX: 0.03, DX: 0.015, Mega: 0.005 },        // absolute, sums to 5%
    variants: {
      EX:   { hpMax: 260, dmgMax: 260, label: 'EX',   art: 'crackling electric-blue energy, radiant silver aura, sparks flying' },
      DX:   { hpMax: 300, dmgMax: 300, label: 'DX',   art: 'wreathed in golden fire, glowing prismatic divine radiance' },
      Mega: { hpMax: 340, dmgMax: 340, label: 'MEGA', art: 'colossal mega-evolved form, swirling cosmic nebula background, rainbow prismatic aura, glowing mega stone on its brow' },
    },
  },
};
...
function stageShape(stageNo, variant) {
  ...
  const [hpLo, hpHi] = STAGES.hp[stageNo];
  const [dmgLo, dmgHi] = STAGES.dmg[stageNo];
  const hpMax = v ? v.hpMax : hpHi;
  const dmgMax = v ? v.dmgMax : dmgHi;
  return `{
...
// hp/damage into [floor, ceiling] (variant ceiling when set), rounded to multiples
// of 10, and evolutions must beat the prior stage's HP by at least 10.
function clampStage(stage, stageNo, variant, prevHp) {
  const v = variant ? STAGES.special.variants[variant] : null;
  const clamp10 = (x, lo, hi) => Math.min(hi, Math.max(lo, Math.round((+x || 0) / 10) * 10));
  const [hpLo, hpHi] = STAGES.hp[stageNo];
  const [dmgLo, dmgHi] = STAGES.dmg[stageNo];
  stage.hp = clamp10(stage.hp, hpLo, v ? v.hpMax : hpHi);
  if (prevHp) stage.hp = Math.max(stage.hp, prevHp + 10);
  for (const m of stage.moves) m.damage = clamp10(m.damage, dmgLo, v ? v.dmgMax : dmgHi);
  return stage;
}
