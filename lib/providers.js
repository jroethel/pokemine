...
const providers = {
  ...
  bridge: {
    ...
    async generate({ prompt }) {
      ...
      fs.writeFileSync(p('json'), JSON.stringify({ id, prompt, createdAt: new Date().toISOString() }));
      ...
      cleanup();
      throw new Error('bridge: timed out after 120s waiting for the browser driver ' +
