...
// Hard delete (not the soft archive): clean up an orphan card whose art write
// failed after create() already persisted the record, leaving a card with no image.
function remove(id) {
  fs.rmSync(dir(id), { recursive: true, force: true });
}

// Undo the most recent evolution: drop the last stage and its art (+ .v1 backup),
// keeping earlier stages intact. Caller (server.js) validates there's more than one
// stage; a middle stage can't be dropped without orphaning later stages' continuity.
function deleteLastStage(id) {
  const record = get(id);
  const stage = record.stages.pop();
  const art = path.join(dir(id), stage.art);
  fs.rmSync(art, { force: true });
  fs.rmSync(art.replace(/(\.[a-z]+)$/, '.v1$1'), { force: true });
  return save(record);
}
...
module.exports = {
  init, list, get, create, save, saveArt, readArt, backupArt, remove, deleteLastStage, archive, nextNumber, migrateNumbers, renameFor, root: () => root,
  allocCollector,
